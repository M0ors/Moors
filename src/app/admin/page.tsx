...
export default async function AdminPage() {
  ...
  const [{ data: users, error }, { data: pendingImages }, { data: accessRequests }] =
    await Promise.all([
      supabase
        .from("profiles")
  ...
  return (
    <main>
      ...
      <p className="text-sm text-neutral-600 mb-8">
        Moderate images, review access requests, and manage users
              ...
              return (
                <li key={request.id} className="border rounded p-4">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3 items-center">
                    <form action={reviewAccessRequest} className="flex flex-wrap gap-2 items-center">
                      <input type="hidden" name="request_id" value={request.id} />
                      <input type="hidden" name="status" value="approved" />
                      <label className="text-sm flex items-center gap-1">
                        <input type="checkbox" name="enable_nsfw" defaultChecked />
                        Enable NSFW
                      </label>
                      <button type="submit">Approve</button>
                    </form>
